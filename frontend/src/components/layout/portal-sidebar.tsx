...
function SidebarSection({
  ...
  return (
    <section>
      <p className="mb-2 px-3.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted">
        {section.label}
      </p>
...
export function PortalSidebar(props: PortalSidebarProps) {
  ...
  return (
    <aside className="hidden border-r border-line bg-white text-ink lg:h-full lg:flex lg:flex-col lg:overflow-hidden">
      <Link
        href="/portal"
        onClick={() => onNavigate("/portal")}
        onMouseEnter={() => onPrepareNavigate("/portal")}
        onFocus={() => onPrepareNavigate("/portal")}
        className="flex h-[86px] items-center gap-3 border-b border-line px-4"
      >
        <CompanyBrand />
      </Link>
      ...
      <div className="mt-auto border-t border-line bg-white p-4">
        ...
        <p className="mt-0.5 text-xs capitalize text-body">
          {user.role} · {user.branch}
        </p>
        <div className="mt-3 flex gap-2">
          <button
            onClick={onSignOut}
            className="flex min-h-9 items-center gap-1 rounded-lg px-2 text-xs font-medium text-body hover:bg-[#f2f6fb]"
          >
            <LogOut className="size-3" /> Sign out
          </button>
          <button
            onClick={onReset}
            className="ml-auto flex min-h-9 items-center gap-1 rounded-lg px-2 text-xs text-body hover:bg-[#f2f6fb]"
          >
